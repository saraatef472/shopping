...
<body>
    ...
    <a href="seeall2.php?name=<?php echo $row['brand']?>" class="dropdown-item" name="details"><?php echo $row['brand']?></a>                    
                   ...
                   <a href="../tradein/device.php" class="nav-item nav-link">trade_in</a>
                <div class="nav-item dropdown">
...
<!-- <br> -->
<div class="alll">
<?php $result = mysqli_query($conn, "SELECT * FROM `products` WHERE type = 'mobile' or type = 'mobiles' or type = 'mob' or type = 'iphone' or description = 'mobile' or description = 'mobiles'");
    if($result->num_rows>0){
        while ($row = mysqli_fetch_array($result)) {
...
<?php  echo ' <image class="card-img-top"  src="data:image/jpeg;base64,' . base64_encode($row['image']) . '"/>'?>
  <div class="card-body">
    ...
    <a href="<?php echo $row['url']?>" class="btn btn-primary" name="details">Details</a>
  </div>
</div>
