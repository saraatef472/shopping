...
$result = mysqli_query($conn, "SELECT DISTINCT brand, device, image2, model, subseries,processor,ram FROM `trade_in` WHERE device ='$device' AND brand ='$brand' AND model = '$model' AND ram <> ''");
    if($result->num_rows>0){
...
<a href="storage2.php?brand=<?php echo $row['brand'];?>&model=<?php echo $row['model']; ?>&processor=<?php echo $row['processor']; ?>&device=<?php echo $row['device'];?>&ram=<?php echo $row['ram']; ?>&subseries=<?php echo $row['subseries']; ?>&#right">

<h5 class="card-title"><?php echo $row['ram'];?></h5>
 ...
 //echo "$device";
 echo"<script> window.location.href='storage2.php?device=$device&brand=$brand&model=$model&subseries=$subseries&processor=$processor&ram=$ram&#right'</script>";
    }?></div>
</div>
<!-- end second section -->
