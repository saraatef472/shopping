...
<h6>Select subseries</h6>

<div class="alll">
<BR>
<?php if (isset($_GET['device']) && isset($_GET['brand']) && isset($_GET['model'])) {
...
$sql = "SELECT DISTINCT brand, device, image2, model, processor ,ram, subseries FROM `trade_in` WHERE device ='$device' AND  brand ='$brand'AND model ='$model' AND subseries <> ''";
$result = mysqli_query($conn, $sql);
...
<a href="processor2.php?brand=<?php echo $brand?>&model=<?php echo $model?>&device=<?php echo$device?> &subseries=<?php echo $row['subseries']; ?>&#right">

<h5 class="card-title"><?php echo $row['subseries'];?></h5>

</a>

    </form>
    </div><?php }} else{
 echo"<script> window.location.href='processor2.php?device=$device&brand=$brand&model=$model&subseries=$subseries&#right'</script>";
