...
$result = mysqli_query($conn, "SELECT DISTINCT brand, device,image2  FROM `trade_in` WHERE device = '$device' ");
    if($result->num_rows>0){
        while ($row = mysqli_fetch_array($result)) {
...
<a href="model2.php?device=<?php echo $row['device']; ?>&brand=<?php echo $row['brand']; ?>&#right" name="details">
<div class="card"id="card">
<?php  echo ' <image class="card-img-top"  src="data:image/jpeg;base64,' . base64_encode($row['image2']) . '"/>'?>
</div>
</a>
    </form>
    </div>
