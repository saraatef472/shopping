...
<BR>
<?php
if (isset($_GET['device']) && isset($_GET['brand'])&& isset($_GET['model'])&& isset($_GET['subseries'])) {
...
$sql = "SELECT DISTINCT brand, device,image2,processor,subseries  FROM `trade_in` WHERE device = '$device' AND brand = '$brand'AND model ='$model' AND processor <> ''";
$result = mysqli_query($conn, $sql);
    if($result->num_rows>0){
        while ($row = mysqli_fetch_array($result)) {
...
<a href="ram2.php?device=<?php echo $device ?>&brand=<?php echo $brand?>&model=<?php echo $model ?>&processor=<?php echo $row['processor']?>&subseries=<?php echo $subseries?>&#right" name="details">

<h5 class="card-title"><?php echo $row['processor'];?></h5>

</a>

    </form>
    </div>  <?php }} else{
//echo $brand.$device.$subseries;
echo"<script> window.location.href='ram2.php?device=$device&brand=$brand&model=$model&subseries=$subseries&#right'</script>";
    }?>
</div>
</div>
<!-- end second section -->
