...
else if(isset($_GET['device'])  && isset($_GET['brand'])&& isset($_GET['model']) && isset($_GET['processor']) && isset($_GET['subseries'])){
    $device = $_GET['device'];
  $processor =  $_GET['processor'];
    $brand = $_GET['brand'];
    $model = $_GET['model'];
...
else if(isset($_GET['device'])  && isset($_GET['brand'])&& isset($_GET['model']) && isset($_GET['subseries'])){
    $device = $_GET['device'];
  //$processor =  $_GET['processor'];
    $brand = $_GET['brand'];
    $model = $_GET['model'];
...
$result = mysqli_query($conn, "SELECT DISTINCT brand, device, image2, model, subseries,processor,ram,storage FROM `trade_in` WHERE device ='$device' AND brand ='$brand' AND model = '$model' AND storage <> ''");
    if($result->num_rows>0){
...
<a href="result2.php?brand=<?php echo $row['brand'];?>&model=<?php echo $row['model']; ?>&processor=<?php echo $row['processor']; ?>&device=<?php echo $device ?>&storage=<?php echo $row['storage']; ?>&ram=<?php echo $row['ram']; ?>&subseries=<?php echo $row['subseries']; ?>&#right">


<h5 class="card-title"><?php echo $row['storage'];?></h5>

</a>
</form>
    </div><?php }}else{
       // echo $device;
echo"<script> window.location.href='result2.php?brand= $brand&model= $model&processor= $processor&device=$device&storage=$storage&ram=$ram&#right'</script>";
    }?></div>
</div>
<!-- end second section -->
